...
export function createElement(id, x1, y1, x2, y2, type, width, strokeColor) {
  ...
  switch (type) {
    case TOOLS.LINE:
      roughElement = generator.line(x1, y1, x2, y2, colorObj);
      break;
    case TOOLS.RECTANGLE:
      const height = x2 - x1;
      const width = y2 - y1;
      roughElement = generator.rectangle(x1, y1, height, width, colorObj);
      break;
    case TOOLS.CIRCLE:
      const diameter =
        2 * Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2));
      roughElement = generator.circle(x1, y1, diameter, colorObj);
      break;
    case TOOLS.TRIANGLE:
